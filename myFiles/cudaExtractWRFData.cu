...
void readData(FILE* f, vector<string> strCurrentDay, string filePath1){
        ...
        if (blnParamArr[msg_count] == true){
            ...
            if (flag == true){
                ...
                for(int i =0; i<numStations; i++){
                    ...
                    station->closestPoint[threadIdx.x] = closestPoint;
                }
                flag = false;
            }
            ...
            for (int i=0; i<numStations; i++){
                ...
                double dblCurrentValue = values[station->closestPoint[threadIdx.x]];
                sem_wait(&valuesProtection[i]);
                station->values[threadIdx.x][index] = dblCurrentValue;
    ...
    mapData(strCurrentDay.at(3), hour, threadIdx.x);
